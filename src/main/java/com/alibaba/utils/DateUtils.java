...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
    ...
    public static final String FORMAT_UTC_PATTERN = "yyyy-MM-dd'T'HH:mm:ss'Z'";
     ...
     * @Description ：返回指定天数之后的日期
     */
    public static Date afterDays(String datestr, int days) {
...
//	}
     ...
     * @return
     */
    public static Date getUTC(String datestr, String pattern) {
     ...
     * @return
     */
    public static Date getFromUTC(String datestr, String pattern) {
        Date date = parseDate(datestr, pattern);
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
        cal.add(Calendar.HOUR_OF_DAY, 8);
     ...
     * @return
     */
    public static Date getFromUTC(Date date) {
        Calendar cal = Calendar.getInstance();
        cal.setTime(date);
        cal.add(Calendar.HOUR_OF_DAY, 8);
        return cal.getTime();
...
//	}
     ...
     * @return
     */
    public static Date getSecondAfter(Date date, Long count) {
        long time = date.getTime();
        long diff = count * 1000;
        long nowTime = time + diff;
        return new Date(nowTime);
...
//        calendar.set(Calendar.MILLISECOND, 999);
     ...
     * 根据时长秒数 获取 时：分：秒 格式
     */
    public static String getHourMinuteSecond(Long timeSeconds) {
        Long hours = new Double(Math.floor(new Double(timeSeconds) / 3600)).longValue();
        Long minutes = new Double(Math.floor((new Double(timeSeconds) % 3600) / 60)).longValue();
        Long seconds = new Double(Math.floor(new Double(timeSeconds) % 60)).longValue();
        return (hours < 10L ? ("0" + hours) : hours) + ":" + (minutes < 10L ? ("0" + minutes) : minutes) + ":" + (seconds < 10L ? ("0" + seconds) : seconds);
    }
     ...
     * 两个时间返回数组
     *
     * @param fetureTime 2020-10-09
     * @param afterTime  2020-09-09
     * @return 返回 2020-09-09 到 2020-10-09 数组
     */
    public static List<String> handleCirculationDate(String fetureTime, String afterTime) {
        List<String> listDate = new ArrayList<>();
        listDate.add(fetureTime);
     ...
     * @return
     */
    public static long getTodayTime2endSeconds() {
        Date today = new Date();
        Date nextDay = addDays(today, 1);
        nextDay = parseDate(formatDate(nextDay, FORMAT_YYYY_MM_DD) + TIMEZERO_BLANK);
        long seconds = diffSeconds(nextDay, today);
        return seconds;
    }
